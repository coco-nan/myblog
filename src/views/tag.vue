...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      ...
      alert("确定删除？")
      this.$axios.post(api.tag.remove,{
        id:row.id
      }).then(data =>{
        if(data.code == 200){
          alert('删除成功')
    ...
    pre() {
      if (this.pageIndex > 1) {
        this.pageIndex -= 1;
        this.$axios
          .post(api.tag.get, { pageIndex: this.pageIndex })
          .then((data) => {
    ...
    add() {
      this.$axios
        .post(api.tag.add, { name: this.sub_type_name })
        .then((data) => {
          if (data.code == 200) {
            alert("添加成功");
    ...
    // 获取数据
    getData() {
      this.$axios.post(api.tag.get, { pageIndex: 1 }).then((data) => {
        this.tableData = data.data;
